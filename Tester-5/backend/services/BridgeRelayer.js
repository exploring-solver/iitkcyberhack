...
class BridgeRelayer {
    constructor({ config }) {
        ...
        // Initialize Web3 instances with proper configs
        this.networks = {
        ...
        // Properly add accounts to web3 instances
        const amoyAccount = this.networks.amoy.eth.accounts.privateKeyToAccount(
            config.amoy.relayerPrivateKey
        );
        const sepoliaAccount = this.networks.sepolia.eth.accounts.privateKeyToAccount(
            config.sepolia.relayerPrivateKey
        );

        this.networks.amoy.eth.accounts.wallet.add(amoyAccount);
        this.networks.sepolia.eth.accounts.wallet.add(sepoliaAccount);
    ...
    async relayTransfer(targetChain, transferData, transferId) {
        try {
            ...
            // Get latest block and nonce
            const nonce = await this.networks[targetChain].eth.getTransactionCount(account);

            // Prepare transaction data
            const data = contract.methods[method](
                transferData.user,
                transferData.amount.toString(),
                proof,
                transferId
            ...
            // Set gas parameters based on chain
            if (targetChain === 'sepolia') {
                const block = await this.networks[targetChain].eth.getBlock('latest');
                const baseFee = BigInt(block.baseFeePerGas);
                const priorityFee = BigInt('25000000000'); // 25 Gwei
                
                tx.maxPriorityFeePerGas = priorityFee;
                tx.maxFeePerGas = baseFee * 2n + priorityFee;
            } else {
                // For Amoy (legacy transaction)
                const gasPrice = await this.networks[targetChain].eth.getGasPrice();
                tx.gasPrice = BigInt(gasPrice) * 2n; // Double the gas price for Amoy
            }
            ...
            tx.gas = Math.ceil(Number(gasEstimate) * 1.2); // 20% buffer

            // Sign and send transaction
            const signedTx = await this.networks[targetChain].eth.accounts.signTransaction(
                tx,
                this.config[targetChain].relayerPrivateKey
            );
            
            const receipt = await this.networks[targetChain].eth.sendSignedTransaction(
    ...
    async updateMerkleTree(chain, transfers) {
        try {
            ...
            const data = contract.methods.updateMerkleRoot(root).encodeABI();
            
            // Get the nonce
            const nonce = await this.networks[chain].eth.getTransactionCount(account);
            
            // Create and sign transaction with proper gas parameters
            const tx = {
                from: account,
                to: contract.options.address,
                nonce: nonce,
            ...
            if (chain === 'sepolia') {
                // Get latest block
                const block = await this.networks[chain].eth.getBlock('latest');
                const baseFee = BigInt(block.baseFeePerGas);
                
                // Set higher priority fee for Sepolia
                const priorityFee = BigInt('25000000000'); // 25 Gwei
                
                tx.maxPriorityFeePerGas = priorityFee;
                tx.maxFeePerGas = baseFee * 2n + priorityFee; // 2x base fee + priority fee
                
                // Estimate gas with buffer
                const gasEstimate = await contract.methods.updateMerkleRoot(root).estimateGas({
                    from: account
                });
                tx.gas = Math.ceil(Number(gasEstimate) * 1.2); // 20% buffer
            } else {
                // For non-EIP1559 chains (amoy)
                const gasPrice = await this.networks[chain].eth.getGasPrice();
                const gasEstimate = await contract.methods.updateMerkleRoot(root).estimateGas({
                    from: account
                });
                
                tx.gasPrice = BigInt(gasPrice) * 2n; // Double the current gas price
                tx.gas = Math.ceil(Number(gasEstimate) * 1.2);
            }
            
            // Sign and send the transaction
            const signedTx = await this.networks[chain].eth.accounts.signTransaction(
                tx,
                this.config[chain].relayerPrivateKey
            );
            
            const receipt = await this.networks[chain].eth.sendSignedTransaction(
                signedTx.rawTransaction
            );

            console.log('Merkle root updated on chain, tx:', receipt.transactionHash);
            return receipt;
        } catch (error) {
